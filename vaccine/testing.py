...
class AppTester:
    ...
    def assert_state(self, name: Optional[str]):
        ...
        """
        assert self.user.state.name == name, (
    ...
    def assert_answer(self, answer_name: str, answer_value: str):
        ...
        assert self.user.answers[answer_name] == answer_value, (
    ...
    def assert_no_answer(self, answer_name: str):
        ...
        """
        assert self.user.answers.get(answer_name) is None, (
    ...
    def assert_metadata(self, metadata_name: str, metadata_value: Any):
        ...
        """
        assert metadata_name in self.user.metadata, (
            f"{metadata_name} not in user metadata"
        )

        actual_value = self.user.metadata[metadata_name]
        assert self.user.metadata[metadata_name] == metadata_value, (
    ...
    def assert_num_messages(self, num: int):
        ...
        """
        assert len(self.application.messages) == num, (
    ...
    def assert_message(
        ...
        if session is not None:
            assert message.session_event == session, (
        ...
        if max_length is not None:
            assert len(str(message.content)) <= max_length, (
