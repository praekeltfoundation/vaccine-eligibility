...
async def test_state_timed_out():
    u = User(
        addr="27820001001", state=StateData(name="state_medical_aid"), session_id=1
    )
    app = Application(u)
    ...
    [reply] = await app.process_message(msg)
    assert u.state.name == "state_timed_out"
    assert len(reply.content) < 140
    assert u.answers["resume_state"] == "state_medical_aid"
    assert u.session_id != 1


async def test_state_timed_out_restart():
    u = User(
        addr="27820001001",
        state=StateData(name="state_timed_out"),
        session_id=1,
        answers={"resume_state": "state_medical_aid"},
    )
    app = Application(u)
    msg = Message(
        content="start over",
...
async def test_state_timed_out_resume():
    u = User(
        addr="27820001001",
        state=StateData(name="state_timed_out"),
        session_id=1,
...
async def test_state_timed_out_error():
    u = User(
        addr="27820001001",
        state=StateData(name="state_timed_out"),
        session_id=1,
